...
#ifdef __CUDACC__
#define PUERHLAB_RAW_NORM_HD __host__ __device__
#else
#define PUERHLAB_RAW_NORM_HD
#endif
...
PUERHLAB_RAW_NORM_HD inline auto Clamp01(const float value) -> float {
  return value < 0.0f ? 0.0f : (value > 1.0f ? 1.0f : value);
}

PUERHLAB_RAW_NORM_HD inline auto NormalizeSample(const float sample, const float black,
...
PUERHLAB_RAW_NORM_HD inline auto RelativeWhiteBalanceMultiplier(const float* wb_multipliers,
                                                                const int color_idx,
