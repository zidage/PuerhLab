...
const std::string Queries::init_table_query =
    ...
    "TIMESTAMP, modified_time "
    "TIMESTAMP);"
    "CREATE TABLE Version (hash BIGINT PRIMARY KEY, history_id BIGINT, parent_hash BIGINT, content "
    "JSON)";
...
const std::string Queries::element_insert_query =
    "INSERT OR REPLACE INTO Element (id,type,element_name,added_time,modified_time,ref_count) "
    "VALUES "
    "(?,?,?,?,?,?);";
...
const std::string Queries::element_update_query =
    "UPDATE Element SET type = ?, element_name = ?, added_time = ?, modified_time = ?, ref_count = "
    "? WHERE id = ?;";
...
const std::string Queries::folder_content_lookup_query =
