...
class EditorDialog final : public QDialog {
 ...
 private:
  ...
  void UpdateVersionUi() {
    ...
    QString      label    = versioning::MakeTxCountLabel(tx_count);
  ...
  void CommitWorkingVersion() {
    ...
    try {
      ...
      committed_id = controllers::CommitWorkingVersion(history_service_, history_guard_,
  ...
  void StartNewWorkingVersionFromUi() {
    ...
    working_version_ = controllers::SeedWorkingVersionFromLatest(element_id_, history_guard_);
    working_version_.SetBasePipelineExecutor(pipeline_guard_->pipeline_);
    UpdateVersionUi();
  }

  void StartNewWorkingVersionFromCommit(const Hash128& committed_id) {
    working_version_ = controllers::SeedWorkingVersionFromParent(
        element_id_, committed_id, CurrentWorkingMode() != WorkingMode::Plain);
    working_version_.SetBasePipelineExecutor(pipeline_guard_->pipeline_);
  }
