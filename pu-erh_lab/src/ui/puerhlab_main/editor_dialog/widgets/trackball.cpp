...
void CdlTrackballDiscWidget::SetPosition(const QPointF& position) {
  const QPointF clamped = color_wheel::ClampDiscPoint(position);
...
auto CdlTrackballDiscWidget::GetPosition() const -> QPointF { return position_; }

void CdlTrackballDiscWidget::SetPositionChangedCallback(PositionCallback cb) {
  on_position_changed_ = std::move(cb);
}

void CdlTrackballDiscWidget::SetPositionReleasedCallback(PositionCallback cb) {
  on_position_released_ = std::move(cb);
}

void CdlTrackballDiscWidget::resizeEvent(QResizeEvent*) { wheel_cache_ = QImage(); }
...
void CdlTrackballDiscWidget::mousePressEvent(QMouseEvent* event) {
...
void CdlTrackballDiscWidget::mouseMoveEvent(QMouseEvent* event) {
  if (!dragging_) {
...
void CdlTrackballDiscWidget::mouseReleaseEvent(QMouseEvent* event) {
  if (event->button() != Qt::LeftButton) {
...
void CdlTrackballDiscWidget::mouseDoubleClickEvent(QMouseEvent* event) {
  if (event->button() != Qt::LeftButton) {
...
auto CdlTrackballDiscWidget::DiscRect() const -> QRectF {
...
auto CdlTrackballDiscWidget::ToWidgetPoint(const QPointF& normalized, const QRectF& disc) const
    -> QPointF {
...
auto CdlTrackballDiscWidget::ToNormalizedPoint(const QPointF& widget_point, const QRectF& disc) const
    -> QPointF {
  const float radius = static_cast<float>(disc.width() * 0.5);
  if (radius <= 0.0f) {
...
void CdlTrackballDiscWidget::UpdateFromMouse(const QPointF& widget_point, bool emit_changed,
                                             bool emit_release) {
...
void CdlTrackballDiscWidget::EnsureWheelCache(const QRectF& disc) {
