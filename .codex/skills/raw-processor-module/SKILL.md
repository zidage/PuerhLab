...
description: Use when modifying the RAW Processor module in pu-erh_lab, shared Metal GPU utilities, or Metal RAW shaders and their CMake wiring.
---
...
- Keep shared Metal image geometry helpers such as crop, resize, and warp outside `edit/operators/`; place them under `pu-erh_lab/src/metal/metal_utils/` with a dedicated utility name such as `geometry_utils`, and keep operators focused on orchestration.
