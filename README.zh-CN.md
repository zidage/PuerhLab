...
- Lensfun 的 Windows 额外依赖：`GLib2`。启用 vcpkg toolchain 时，Pu-erh Lab 会优先自动使用 `vcpkg/installed/<triplet>` 下的 GLib2；`PUERHLAB_LENSFUN_GLIB2_BASE_DIR` 仅在你需要覆盖该自动检测结果，或使用非 vcpkg 的 GLib2 包时才需要手动指定
