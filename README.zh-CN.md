...
- macOS：当前提供实验性的 CPU-only Qt 主应用构建；编辑器后端默认禁用，后续计划以 Metal 为主要加速方向。
...
- 包管理依赖（Windows 下通常通过 vcpkg toolchain 解析）：`OpenCV`、`Eigen3`、`OpenGL`、`hwy`、`lcms2`、`OpenColorIO`、`OpenImageIO`、`libraw`、`xxHash`、`OpenMP`
- 测试框架：`googletest`（通过 `FetchContent` 获取）
- Windows 本地导入二进制：`DuckDB`、`Exiv2`、`easy_profiler`
- 镜头校正依赖：上游 `Lensfun` 源码子模块位于 `pu-erh_lab/src/third_party/lensfun`，由顶层 CMake 自动构建
- Lensfun 的 Windows 额外依赖：`GLib2`，通过 `PUERHLAB_LENSFUN_GLIB2_BASE_DIR` 指定，默认路径为 `pu-erh_lab/third_party/glib-2.28.1`。该目录不随仓库提供，需要手动准备

### 3）初始化内置 Lensfun 源码

在配置 Pu-erh Lab 之前，先确保 Lensfun 子模块已经拉取：
...
如果 Windows 下的 GLib2 不在 `pu-erh_lab/third_party/glib-2.28.1`，请额外传入：
...
- `macos_debug` 和 `macos_release` preset 默认关闭 CUDA、OpenGL 编辑器和测试，仅构建可运行的主 Qt 应用。
- 如果 Homebrew 不在默认前缀下，请在配置时额外传入 `-DCMAKE_PREFIX_PATH=/path/to/prefix`。
- 当前 macOS 构建目标是保证主应用在不依赖 CUDA/OpenGL 的情况下可以配置、编译和运行；触发编辑器入口时会明确提示当前构建未包含编辑器后端。
- 后续 macOS 加速与图形后端会优先朝 Metal 方向演进，而不是继续依赖 CUDA/OpenGL。
