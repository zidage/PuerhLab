...
struct LookPipelineAdapter {
  static auto Load(CPUPipelineExecutor& exec, const LookAdjustmentState& base)
    ...
    CopyLookStateToLegacy(base, legacy_base);
    auto [loaded_legacy, has_any] = pipeline_io::LoadStateFromPipeline(exec, legacy_base);
    return {LookStateFromLegacy(loaded_legacy), has_any};
  }
  ...
  static auto FieldChanged(AdjustmentField field, const LookAdjustmentState& current,
 ...
 private:
  static void CopyLookStateToLegacy(const LookAdjustmentState& look, AdjustmentState& legacy) {
    legacy.hls_target_hue_              = look.hls_target_hue_;
    legacy.hls_hue_adjust_              = look.hls_hue_adjust_;
    legacy.hls_lightness_adjust_        = look.hls_lightness_adjust_;
    legacy.hls_saturation_adjust_       = look.hls_saturation_adjust_;
    legacy.hls_hue_range_               = look.hls_hue_range_;
    legacy.lift_wheel_                  = look.lift_wheel_;
    legacy.gamma_wheel_                 = look.gamma_wheel_;
    legacy.gain_wheel_                  = look.gain_wheel_;
    legacy.hls_hue_adjust_table_        = look.hls_hue_adjust_table_;
    legacy.hls_lightness_adjust_table_  = look.hls_lightness_adjust_table_;
    legacy.hls_saturation_adjust_table_ = look.hls_saturation_adjust_table_;
    legacy.hls_hue_range_table_         = look.hls_hue_range_table_;
    legacy.lut_path_                    = look.lut_path_;
  }

  static auto LookStateFromLegacy(const AdjustmentState& legacy) -> LookAdjustmentState {
    LookAdjustmentState look;
    look.hls_target_hue_              = legacy.hls_target_hue_;
    look.hls_hue_adjust_              = legacy.hls_hue_adjust_;
    look.hls_lightness_adjust_        = legacy.hls_lightness_adjust_;
    look.hls_saturation_adjust_       = legacy.hls_saturation_adjust_;
    look.hls_hue_range_               = legacy.hls_hue_range_;
    look.lift_wheel_                  = legacy.lift_wheel_;
    look.gamma_wheel_                 = legacy.gamma_wheel_;
    look.gain_wheel_                  = legacy.gain_wheel_;
    look.hls_hue_adjust_table_        = legacy.hls_hue_adjust_table_;
    look.hls_lightness_adjust_table_  = legacy.hls_lightness_adjust_table_;
    look.hls_saturation_adjust_table_ = legacy.hls_saturation_adjust_table_;
    look.hls_hue_range_table_         = legacy.hls_hue_range_table_;
    look.lut_path_                    = legacy.lut_path_;
    return look;
  }
};
