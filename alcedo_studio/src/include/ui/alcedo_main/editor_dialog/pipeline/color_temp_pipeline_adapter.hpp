...
struct ColorTempPipelineAdapter {
  static auto Load(CPUPipelineExecutor& exec, const ColorTempAdjustmentState& base)
    ...
    AdjustmentState legacy_base =
        ToLegacyAdjustmentState(EditorAdjustmentSnapshot{.color_temp_ = base});
    auto [loaded_legacy, has_any] = pipeline_io::LoadStateFromPipeline(exec, legacy_base);
    return {FromLegacyAdjustmentState(loaded_legacy).color_temp_, has_any};
  }
  ...
  static auto FieldChanged(AdjustmentField field,
                           const ColorTempAdjustmentState& current,
